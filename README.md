...
The train and test sets were too large to be uploaded as single files so they were broken down into smaller pieces. They can be found in the train and test folders. These smaller pieces can be combined together by using the combineData function that is in combData.py file. 

## ML MODELS
At the moment logistic regression, Naive Bayes and Neural Network models are trained for each tag seperately. For future work, a multi-task learning model will be implemened. Also LSTMs can be another approach that can give more promising results. The models were compared based on accuracy on hold-out validation dataset.  
